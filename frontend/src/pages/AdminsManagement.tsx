...
const AdminsManagement: React.FC = () => {
  ...
  return <div className="page-container admins-management">
    ...
    <div style={{ display: 'flex', gap: '1rem', marginBottom: '1.5rem', flexWrap: 'wrap', alignItems: 'center', justifyContent: 'space-between' }}>
      <div style={{ display: 'flex', gap: '0.5rem', flexWrap: 'wrap' }}>{([['ALL', 'Todos los administradores'], ['ACTIVE', 'Alta'], ['INACTIVE', 'Baja']] as const).map(([value, label]) => <button key={value} type="button" onClick={() => setStatusFilter(value)} style={{ padding: '0.45rem 0.85rem', borderRadius: 16, border: statusFilter === value ? '1px solid var(--primary)' : '1px solid var(--border)', background: statusFilter === value ? 'var(--primary-light)' : 'var(--surface)', color: statusFilter === value ? 'var(--primary-text)' : 'var(--text-muted)', fontWeight: statusFilter === value ? 700 : 500, fontSize: '0.84rem', cursor: 'pointer' }}>{label}</button>)}</div>
      <div style={{ position: 'relative', flex: '1 1 240px', maxWidth: 380 }}><Search size={17} style={{ position: 'absolute', left: 12, top: '50%', transform: 'translateY(-50%)', color: 'var(--text-muted)' }} /><input type="text" placeholder="Buscar por nombre, DNI, teléfono o correo..." value={searchTerm} onChange={(event) => setSearchTerm(event.target.value)} style={{ ...inputStyle, paddingLeft: '2.4rem' }} /></div>
    </div>

    <div className="glass-panel" style={{ padding: 0, overflow: 'hidden' }}>
      <div className="table-responsive">
        <table style={{ width: '100%', minWidth: 820, borderCollapse: 'collapse', textAlign: 'left' }}>
          <thead><tr style={{ background: 'var(--surface-alt)', borderBottom: '1px solid var(--border)' }}>{['ADMINISTRADOR Y CONTACTO', 'DNI / NIE', 'ESTADO', 'ACCIONES'].map((heading) => <th key={heading} style={{ padding: '1rem 1.25rem', color: 'var(--text-muted)', fontWeight: 600, fontSize: '0.84rem', textAlign: heading === 'ACCIONES' ? 'right' : 'left' }}>{heading}</th>)}</tr></thead>
          <tbody>
            {loading ? <tr><td colSpan={4} style={{ padding: '3rem', textAlign: 'center', color: 'var(--text-muted)' }}>Cargando administradores...</td></tr> : filteredAdmins.length === 0 ? <tr><td colSpan={4} style={{ padding: '3rem', textAlign: 'center', color: 'var(--text-muted)' }}>{searchTerm ? 'No se encontraron administradores con ese criterio.' : 'No hay administradores registrados en el sistema.'}</td></tr> : filteredAdmins.map((admin) => {
              const initials = `${admin.profile?.firstName?.[0] || ''}${admin.profile?.lastName?.[0] || ''}`.toUpperCase() || 'AD';
              const isActive = admin.status === 'ACTIVE';
              const isSelf = admin.id === currentUserId;
              return <tr key={admin.id} style={{ borderBottom: '1px solid var(--border)' }}>
                <td style={{ padding: '1rem 1.25rem' }}><div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}><div style={{ width: 38, height: 38, borderRadius: '50%', background: 'var(--primary)', color: '#fff', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 'bold', fontSize: '0.85rem' }}>{initials}</div><div><div style={{ fontWeight: 600, color: 'var(--text-main)', fontSize: '0.95rem' }}>{admin.profile?.firstName} {admin.profile?.lastName}{isSelf && <span style={{ marginLeft: 8, fontSize: '0.72rem', color: 'var(--primary)', fontWeight: 700 }}>(Tú)</span>}</div><div style={{ color: 'var(--text-muted)', fontSize: '0.8rem' }}>{admin.email}</div>{admin.profile?.phone && <div style={{ color: 'var(--primary)', fontSize: '0.78rem', display: 'flex', alignItems: 'center', gap: '0.25rem' }}><Phone size={12} /> {admin.profile.phone}</div>}</div></div></td>
                <td style={{ padding: '1rem 1.25rem', color: 'var(--text-main)', fontSize: '0.88rem' }}>{admin.profile?.dni || <span style={{ color: 'var(--text-light)', fontStyle: 'italic' }}>No asignado</span>}</td>
                <td style={{ padding: '1rem 1.25rem' }}><div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', flexWrap: 'wrap' }}><span style={{ padding: '0.35rem 0.65rem', background: isActive ? '#dcfce7' : '#fee2e2', color: isActive ? '#166534' : '#991b1b', borderRadius: 16, fontSize: '0.75rem', fontWeight: 600 }}>{isActive ? 'Alta' : 'Baja'}</span><button type="button" disabled={isSelf && isActive} onClick={() => toggleStatus(admin)} style={{ padding: '0.3rem 0.55rem', borderRadius: 6, border: `1px solid ${isActive ? '#fca5a5' : '#86efac'}`, background: isActive ? '#fff1f2' : '#f0fdf4', color: isActive ? '#b91c1c' : '#15803d', cursor: isSelf && isActive ? 'not-allowed' : 'pointer', opacity: isSelf && isActive ? 0.55 : 1, fontSize: '0.75rem', fontWeight: 600 }}>{isActive ? 'Dar de baja' : 'Dar de alta'}</button></div></td>
                <td style={{ padding: '1rem 1.25rem', textAlign: 'right' }}><div style={{ display: 'flex', justifyContent: 'flex-end', gap: '0.45rem' }}><button type="button" onClick={() => setViewingAdmin(admin)} title="Ver ficha de administrador" style={iconButtonStyle}><Eye size={16} /></button><button type="button" onClick={() => openEdit(admin)} title="Editar ficha de administrador" style={iconButtonStyle}><Edit2 size={16} /></button><button type="button" disabled={isSelf} onClick={() => setDeletingAdmin(admin)} title="Eliminar administrador" style={{ ...iconButtonStyle, cursor: isSelf ? 'not-allowed' : 'pointer', opacity: isSelf ? 0.55 : 1 }}><Trash2 size={16} /></button></div></td>
              </tr>;
            })}
          </tbody>
        </table>
