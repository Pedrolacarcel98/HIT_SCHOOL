...
import { Award, FileText, CheckCircle2, ExternalLink } from 'lucide-react';
                ...
                <div key={assignment.id} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '1.25rem', border: '1px solid var(--border)', borderRadius: '12px', background: 'var(--surface-alt)' }}>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
                    {!isTest && (
                      <button
                        type="button"
                        onClick={() => documentUrl && window.open(documentUrl, '_blank', 'noopener,noreferrer')}
                        ...
                        style={{
                          ...
                          color: documentUrl ? 'var(--primary)' : 'var(--text-muted)',
                          padding: '0.45rem 0.75rem',
                          borderRadius: '10px',
                          cursor: documentUrl ? 'pointer' : 'not-allowed',
                          opacity: documentUrl ? 1 : 0.7,
                          fontWeight: 600
                        }}
                      >
                        <ExternalLink size={14} /> {documentUrl ? 'Abrir documento' : 'Sin documento'}
                      </button>
                    )}
